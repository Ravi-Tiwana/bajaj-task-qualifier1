...
public class AppRunner implements CommandLineRunner {
    ...
    private void generateWebhookAndSubmitQuery() {
        ...
        ResponseEntity<?> response = restTemplate.postForEntity(generateWebhookUrl, requestEntity, Map.class);
        ...
        if (response.getStatusCode() == HttpStatus.OK) {
            ...
            if (body instanceof Map) {
                ...
                String webhookUrl = responseBody.get("webhookUrl") != null ? responseBody.get("webhookUrl").toString() : null;
                String accessToken = responseBody.get("accessToken") != null ? responseBody.get("accessToken").toString() : null;
                if (webhookUrl != null && accessToken != null) {
                    // Step 6: Submit the SQL Query
                    submitFinalQuery(webhookUrl, accessToken);
                } else {
                    System.err.println("Response missing webhookUrl or accessToken.");
                }
            } else {
                System.err.println("Unexpected response body type: " + (body != null ? body.getClass() : "null"));
            }
        } else {
